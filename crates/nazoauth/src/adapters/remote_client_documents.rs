...
use crate::adapters::sector_identifier::MAX_RESPONSE_BYTES;
use crate::adapters::sector_identifier::is_blocked_host;
use crate::adapters::sector_identifier::is_blocked_ip;
use crate::adapters::sector_identifier::parse_sector_identifier_document;
...
impl nazo_oauth_server::contracts::dynamic_client_registration::RemoteJwksResolverPort
    for RemoteClientDocumentResolver
{
    fn resolve<'a>(
        &'a self,
        uri: &'a str,
...
impl nazo_oauth_server::ports::remote_request_object::RemoteRequestObjectResolverPort
    for RemoteClientDocumentResolver
{
    fn resolve_request_object<'a>(
        &'a self,
        uri: &'a str,
    ) -> nazo_oauth_server::ports::remote_request_object::RequestObjectFuture<'a> {
        Box::pin(async move { self.request_object(uri).await })
    }
}
