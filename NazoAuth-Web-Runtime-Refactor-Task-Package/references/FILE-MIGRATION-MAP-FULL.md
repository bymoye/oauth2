...
下面的每行源文件均在基线存在。`App/`代表`crates/authorization-server/src/`，`Native/`代表`crates/nazoauth/src/`；表内目标是固定施工位置。“保持Native”仍包含P1从原server到Native的机械移动；不意味着源路径留在原server。

## E1. authorization-server全部生产文件

| 源文件（crates/下） | 行数 | 当前问题/判断 | 固定目标 | 操作 | Phase |
|---|---|---|---|---|---|
...
| 源文件（crates/下） | 固定目标 | 操作 | Phase |
|---|---|---|---|
| key-management/src/lib.rs | 原文件 + Native/adapters/external_signer.rs + Native/jobs/key_lifecycle.rs | 按P3分别移进程/循环，纯验证/状态/fixtures保留 | T03 |
| key-management/src/model.rs | 原文件 + Native/adapters/external_signer.rs + Native/jobs/key_lifecycle.rs | 按P3分别移进程/循环，纯验证/状态/fixtures保留 | T03 |
| key-management/src/database.rs | 原文件 + Native/adapters/external_signer.rs + Native/jobs/key_lifecycle.rs | 按P3分别移进程/循环，纯验证/状态/fixtures保留 | T03 |
| key-management/src/external.rs | 原文件 + Native/adapters/external_signer.rs + Native/jobs/key_lifecycle.rs | 按P3分别移进程/循环，纯验证/状态/fixtures保留 | T03 |
| key-management/src/lifecycle.rs | 原文件 + Native/adapters/external_signer.rs + Native/jobs/key_lifecycle.rs | 按P3分别移进程/循环，纯验证/状态/fixtures保留 | T03 |
| key-management/src/test_support.rs | 原文件 + Native/adapters/external_signer.rs + Native/jobs/key_lifecycle.rs | 按P3分别移进程/循环，纯验证/状态/fixtures保留 | T03 |
| identity/src/ports/authentication.rs | 原文件 | 仅F10七个既有Port Arc转发 | T04 |
| identity/src/ports/registration.rs | 原文件 | 仅F10七个既有Port Arc转发 | T04 |
| identity/src/ports/passkey.rs | 原文件 | 仅F10七个既有Port Arc转发 | T04 |
| identity/src/ports/federation.rs | 原文件 | 仅F10七个既有Port Arc转发 | T04 |
| runtime-capabilities/src/registry.rs | 原文件 | 新增snapshot_store→同一Arc | T04 |
...
| openid4vci/src/lib.rs | 原文件 + 新application.rs | 声明中立contracts module | T02 |
| openid4vp/src/lib.rs | 原文件 + 新application.rs | 声明中立contracts module | T02 |
| persistence-postgres/src/pool.rs | 原文件 | 保持，Tokio属于具体存储Adapter | 不改 |
| openid4vc-http-actix/src/lib.rs | 原文件 + VCI/VP application.rs | 中立定义移动；Endpoint/parser/presentation保留 | T02 |
| openid4vc-http-actix/src/vci.rs | 原文件 + VCI/VP application.rs | 中立定义移动；Endpoint/parser/presentation保留 | T02 |
| openid4vc-http-actix/src/vp.rs | 原文件 + VCI/VP application.rs | 中立定义移动；Endpoint/parser/presentation保留 | T02 |

## E4. 测试与非生产文件移动规则

原server/tests全部先到Native/tests同相对路径（T01）。后续按被测生产模块：domain纯测试→App/tests/unit/domain；token业务→App/tests/unit/token；authorization纯策略→App/tests/unit/authorization；remote/cache/sector tests→Native/tests/unit/adapters；worker业务批次→App tests/delivery_batches.rs，网络/调度→Native/tests/unit/jobs及adapters。混合测试按实际断言拆成唯一两份不同职责的测试体，不复制同一case。

原tests中PostgreSQL/Valkey/进程/HTTP fixtures保持Native；App只迁可独立依赖中立port的fake工具。#[cfg(test)] #[path]相对路径从新生产文件重新计算。新增framework_boundary_transport先在P0旧server装载，P1随迁；jobs/adapters新增外置unit测试必须在其生产module声明，不能只创建文件。

需要更新的非Rust入口固定为：根Cargo.toml/Cargo.lock；H表manifest；scripts/verify_static_contracts.py；scripts/check_persistence_dependency_graph.py；其tests/unit source-policy路径常量；.github/workflows/code-quality.yml与conformance-security.yml中的受影响命令；Containerfile实际按源码布局读取的路径；docs/project/web-runtime-refactor-baseline.md。只有真实引用旧package/module路径的地方改名，不更换CI镜像、TLS策略或发布机制。

## E5. 新增与删除总表

| 操作 | 精确对象 | 阶段 |
|---|---|---|
| 新增module根 | App/contracts/mod.rs、ports/mod.rs、security/mod.rs、workers/mod.rs、token/mod.rs、authorization/mod.rs；Native/lib.rs、launchers/mod.rs、jobs/mod.rs | 随对应Phase |
| 新增App能力 | ports/audit.rs、remote_request_object.rs、mdoc.rs；contracts/oauth_error.rs、request_facts.rs、token_endpoint.rs、authorization_endpoint.rs、device.rs、ciba.rs；services.rs、policy.rs、sessions.rs、rate_limit.rs | T04–T11 |
| 新增Native能力 | launchers/postgres.rs、valkey.rs、object_store.rs；adapters/external_signer.rs、mdoc_signer.rs、ciba_ping_sender.rs、backchannel_logout_sender.rs；jobs/key_lifecycle.rs、ciba_ping.rs、backchannel_logout.rs | T01/T03/T11/T12 |
| 新增owner定义 | key-management/src/external_signer.rs；openid4vci/src/application.rs；openid4vp/src/application.rs | T02/T03 |
| 新增外层提取契约 | http-actix/src/mtls.rs | T06 |
| 删除 | http-actix/src/request_context.rs及其导出；旧位置SendCibaResponse；OAuthJsonErrorFields及extension；key run_lifecycle/stop_lifecycle/watch；旧server Native module/旧Adapter contract导出 | T01–T13 |
| 保持不动 | PG schema/migrations/SQL；Valkey Lua/key/TTL/CAS；已正确objectstore Port；核心crypto算法/普通库API；缓存算法/Moka | 全阶段 |
